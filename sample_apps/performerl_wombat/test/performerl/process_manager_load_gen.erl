-module(process_manager_load_gen).
...
-export([start_load/2, stop_load/1, get_load_duration/0,
         ...
         get_test_name/0, spawn_pocesses/1]).

%%====================================================================
%% API functions
%%====================================================================

get_test_name() -> "Process Manager".
...
setup_run(Size) ->
    Node = 'process_manager@127.0.0.1',
...
% 2 minute and 40 seconds, enough to make sure the builtin metrics
% are collected three times
get_load_duration() ->
    160000.
...
start_load([Node], Size) ->
    ...
    Pids = rpc:call(Node, ?MODULE, spawn_pocesses, [Size]),

    {load_started,[{node_info,{Node,WoNodeId}},
                   {bomb_pids,Pids},
                   {request_data,{ConnPid,RequestId}}]}.
...
stop_load(StopLoadInfo) ->
    ...
    % stops the ProcessManager request
    {ConnPid, RequestId} = proplists:get_value(request_data, StopLoadInfo),
    io:format("~nstopping process manager request~n"),
    wombat_load_lib:ws_unsubscribe_from_request(ConnPid,RequestId),
    ok = performerl_lib:sleep_progress(100),
